...
function show_button_for_group_4_user()
{
    ...
    if (is_user_logged_in() && has_user_made_successful_payment($user_id)) {
        ...
        error_log('User Group Check: ' . print_r($result, true));
            ...
            ?>
                <script type="text/javascript">
                    jQuery(document).ready(function ($) {
                        ...
                        if ($('#menu-item-3607 .sub-menu').length) {
                            ...
                            console.log("Error: Sub-menu does not exist!");
                        }
                    });
                </script>
                <style>
...
function create_abstract_submission_table()
{
    ...
    $sql = "CREATE TABLE IF NOT EXISTS $table_name (
        ...
        downloadable_abstract VARCHAR(255) NULL,
           ...
           present VARCHAR(10) NOT NULL DEFAULT '',
            present_marked_at DATETIME NULL, 
              present_marked_by VARCHAR(255) NULL,
...
function create_certificates_table()
{
    ...
    $sql = "CREATE TABLE IF NOT EXISTS $table_name (
        ...
        status VARCHAR(100) NOT NULL DEFAULT 'pending',
          present VARCHAR(100) NOT NULL DEFAULT 'pending',
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_abstract'])) {
        ...
        foreach ($co_author_ids as $co_author_id) {
            ...
            if ($co_author_uid) {
                ...
                // Check if the co-author has the same active membership
            ...
            WHERE user_id = %d AND status = 'active'",
                ...
                $status = ($is_in_group_4 && $has_same_membership) ? 'Valid' : 'Invalid';

                // Store the result for logging
                $co_authors_group_check_results[$co_author_uid] = $status === 'Valid' ? 'In Group 4 & Membership Match' : 'Invalid';
            } else {

                // If user ID is not found, mark as Not a Member
                $status = 'Not a Member';
                $co_authors_group_check_results[$co_author_id] = 'Not a Member';
            }
            // Insert into co_author_certificates table
            $wpdb->insert(
                "{$wpdb->prefix}co_author_certificates",
                [
                    'user_id' => $co_author_uid ? $co_author_uid : 0,
...
function compare_abstract_shortcode()
{
    ...
    $abstract = $wpdb->get_row(
        $wpdb->prepare("SELECT abstract_title, abstract_description, abstract_title_initial,rc_comment, abstract_description_initial FROM $table_name WHERE id = %d", $abstract_id)
    );
    ...
    ?>
        ...
        <div class="abstract-comparison">
            ...
            <div class="abstract-column">
                <h3>Comment by RC:</h3>
                <p><?php echo $abstract->rc_comment; ?></p>
            </div>
    ...
    if (isset($_GET['id']) && !empty($_GET['id'])) {
        ...
        if ($abstract) {
            ...
            if (isset($_POST['update_abst'])) {
                ...
                $rc_comment = sanitize_textarea_field($_POST['rc_comment']);
                ...
                <div class="wrap">
                    ...
                    <form method="POST">
                        ...
                        <?php if ($is_editable): ?>
                            ...
                            <textarea name="rc_comment" rows="4" cols="50"
                                style="width: 100%; height: 150px; font-size: 16px; font-weight: normal;"></textarea>
                        <?php endif; ?>
...
function view_available_abstracts_callback()
{
    ...
    $query = "SELECT id, author_id, abstract_title, author_name, co_authors, co_authors_ids, reviewer_status, certificate_issued, present, YEAR(created_at) as year
             FROM $table_name 
    ...
    if (!$abstracts) {
        ...
        foreach ($abstracts as $abstract) {
            ...
            $present_status = $abstract->present ? 'yes' : 'no';
            error_log($present_status);
            ...
            if ($num_coauthors === 0) {
                ...
                echo '<td colspan="2">No co-authors</td>';
                echo '<td>No certificate generation</td>';
                echo '<td>Nothing to download</td>';
                echo '<td class="' . esc_attr($review_class) . '">' . esc_html($reviewer_status) . '</td>';
                ...
                if ($present_status == 'yes' && !$certificate_issued) {
                    ...
                    echo '<td>Please Check author attendence</td>';
                }
                ...
                $co_author_present_status = get_present_status($abstract_id, $first_co_author_id);
                error_log($co_author_present_status . 'first' . $abstract_id);
                $class = ($first_cert_status == 'Valid') ? 'valid-member' : 'invalid-member';
                ...
                if ($first_cert_status == 'Valid' && $co_author_present_status == 'present') {
                    if ($certificate_link) {
                        echo '<td><span class="button certificate-issued">Certificate Issued</span></td>';
                    } else {
                        echo '<td><a 
                            href="' . esc_url(admin_url('admin.php?page=view_available_abstracts&issue_co_author_certificate=true&abstract_id=' . $abstract_id . '&co_author_id=' . $first_co_author_id)) . '" 
                            class="button valid-member">Issue Co-Author Certificate</a></td>';
                    }
                } else if ($first_cert_status == 'Valid' && $co_author_present_status == 'pending') {
                    echo '<td>Co-Author Attendance Pending</td>';
                } else {
                    echo '<td class="invalid-member">This user doesn\'t match criteria</td>';
                }
                ...
                if ($certificate_issued) {
                    echo '<td rowspan="' . $num_coauthors . '">
                           <span class="button certificate-issued">Certificate Issued</span>
                          </td>';
                } elseif ($present_status === 'yes') { // Show "Issue Certificate" only if present is 'yes'
                    echo '<td rowspan="' . $num_coauthors . '">
                    ...
                    echo '<td rowspan="' . $num_coauthors . '">Please Check author attendence</td>'; // Show a placeholder if conditions are not met
                }
                ...
                if ($num_coauthors > 1) {
                    for ($i = 1; $i < $num_coauthors; $i++) {
                        ...
                        $co_author_present_statuss = get_present_status($abstract_id, $co_author_id);
                        error_log($co_author_present_statuss . 'after first' . $abstract_id);
                        ...
                        if ($cert_status == 'Valid') {
                            if ($co_author_present_statuss == 'present') {
                                echo '<td><a 
                                    href="' . esc_url(admin_url('admin.php?page=view_available_abstracts&issue_co_author_certificate=true&abstract_id=' . $abstract_id . '&co_author_id=' . $co_author_id)) . '" 
                                    class="button  valid-member">Issue Co-Author Certificate</a></td>';
                            } elseif ($co_author_present_statuss == 'pending') {
                                echo '<td class="pending-attendance">Co-Author Attendance Pending</td>';
                            }
                        } else {
                            echo '<td class="invalid-member">No Criteria Match</td>';
                        }
...
function get_present_status($abstract_id, $co_author_id)
{
    global $wpdb;
    $certificate_table = $wpdb->prefix . 'co_author_certificates';

    $presentstts = $wpdb->get_row(
        $wpdb->prepare(
            "SELECT present 
             FROM $certificate_table 
    ...
    return !empty($presentstts) ? $presentstts->present : 'not found';
}
...
function handle_certificate_issued()
{
    ...
    if (isset($_GET['issue_certificate']) && $_GET['issue_certificate'] == 'true') {
        ...
        // Define table name
        $table_name = $wpdb->prefix . 'abstract_submissions';
        $format_table = $wpdb->prefix . 'certificate_format';
        ...
        // Fetch certificate format data
        $certificate_format = $wpdb->get_row("SELECT * FROM $format_table LIMIT 1");
        ...
        if ($abstract) {
            ...
            $wpdb->update(
                $table_name,
                array(
                    'certificate_issued' => $pdf_file_path,
            ...
            // Generate the final PDF with the certificate format
            $pdf = new FPDF();
            $pdf->AddPage();

            if ($certificate_format->background_option === 'color') {
                $pdf->SetFillColor(hexdec(substr($certificate_format->background_color, 1, 2)), hexdec(substr($certificate_format->background_color, 3, 2)), hexdec(substr($certificate_format->background_color, 5, 2)));
                $pdf->Rect(0, 0, 210, 297, 'F');
            } elseif ($certificate_format->background_option === 'image' && !empty($certificate_format->background_image)) {
                $background_image_path = str_replace($upload_dir['baseurl'] . '/', '', $certificate_format->background_image);
                $background_image_path = $upload_dir['basedir'] . '/' . $background_image_path;
                error_log('Background Image Path: ' . $background_image_path);
                if (file_exists($background_image_path)) {
                    $pdf->Image($background_image_path, 0, 0, 210, 297);
                }
            }

            if (!empty($certificate_format->iss_logo)) {
                $iss_logo_path = str_replace($upload_dir['baseurl'] . '/', '', $certificate_format->iss_logo);
                $iss_logo_path = $upload_dir['basedir'] . '/' . $iss_logo_path;
                error_log('ISS Logo Path: ' . $iss_logo_path);
                if (file_exists($iss_logo_path)) {
                    $pdf->Image($iss_logo_path, 10, 10, 30);
                }
            }

            if (!empty($certificate_format->cohost_logo)) {
                $cohost_logo_path = str_replace($upload_dir['baseurl'] . '/', '', $certificate_format->cohost_logo);
                $cohost_logo_path = $upload_dir['basedir'] . '/' . $cohost_logo_path;
                error_log('Cohost Logo Path: ' . $cohost_logo_path);
                if (file_exists($cohost_logo_path)) {
                    $pdf->Image($cohost_logo_path, 170, 10, 30);
                }
            }
            ...
            $pdf->Cell(0, 10, 'Certificate ID: ' . $certificate_unique_id, 0, 1, 'C');
            $pdf->Ln(10);
...
function generate_and_send_certificate($abstract, $co_author_id, $certificate_table, $abstract_id, $certificate_id = null)
{
    ...
    if ($certificate_format->background_option === 'image' && !empty($certificate_format->background_image)) {
        $background_image_path = str_replace($upload_dir['baseurl'] . '/', '', $certificate_format->background_image);
        $background_image_path = $upload_dir['basedir'] . '/' . $background_image_path;
        error_log('Background Image Path for co-author cert: ' . $background_image_path);

        // Check if the file exists before adding it to the PDF
        if (file_exists($background_image_path)) {
            $pdf->Image($background_image_path, 0, 0, 210, 297);
            error_log('Background image inserted successfully');
        } else {
            error_log('Background image not found: ' . $background_image_path);
        }
    }
    ...
    if (!empty($certificate_format->iss_logo)) {
        $iss_logo_path = str_replace($upload_dir['baseurl'] . '/', '', $certificate_format->iss_logo);
        $iss_logo_path = $upload_dir['basedir'] . '/' . $iss_logo_path;
        error_log('ISS Logo Path co-author cert: ' . $iss_logo_path);

        if (file_exists($iss_logo_path)) {
            $pdf->Image($iss_logo_path, 10, 10, 30);
            error_log('ISS Logo inserted successfully');
        } else {
            error_log('ISS Logo not found: ' . $iss_logo_path);
        }
    }
    ...
    if (!empty($certificate_format->cohost_logo)) {
        $cohost_logo_path = str_replace($upload_dir['baseurl'] . '/', '', $certificate_format->cohost_logo);
        $cohost_logo_path = $upload_dir['basedir'] . '/' . $cohost_logo_path;
        error_log('Cohost Logo Path co-author cert: ' . $cohost_logo_path);

        if (file_exists($cohost_logo_path)) {
            $pdf->Image($cohost_logo_path, 170, 10, 30);
            error_log('Cohost Logo inserted successfully');
        } else {
            error_log('Cohost Logo not found: ' . $cohost_logo_path);
        }
    }
...
function add_user_membership_category_field($user)
{
    $categories = [
        'life_membership' => 'Life Membership Data',
        'ordinary_membership' => 'Ordinary Membership (Indian & Foreign)',
        'rc_membership' => 'RC Membership Data',
        'student_membership' => 'Student Membership (Indian & Foreign)',
    ];

    // Get existing selected values (for Edit User screen)
    $selected_categories = is_object($user) ? get_user_meta($user->ID, 'membership_categories', true) : [];
    ...
    ?>
        ...
        <table class="form-table">
            <tr>
                <th><label for="membership_categories">Select Membership Categories</label></th>

                <td>
                    <?php foreach ($categories as $key => $label): ?>
                        <label>
                            <input type="checkbox" name="membership_categories[]" value="<?php echo esc_attr($key); ?>" <?php checked(in_array($key, $selected_categories)); ?>>
...
// Add fields to Edit User screen
add_action('show_user_profile', 'add_user_membership_category_field');
add_action('edit_user_profile', 'add_user_membership_category_field');

// Add fields to Add New User screen
function add_user_membership_category_field_new($user)
{
    add_user_membership_category_field(null);
}
add_action('user_new_form', 'add_user_membership_category_field_new');

function save_user_membership_category_field($user_id)
{
    if (!current_user_can('edit_user', $user_id)) {
        return false;
    }
    // Sanitize and save selected values
    $selected_categories = isset($_POST['membership_categories']) ? array_map('sanitize_text_field', $_POST['membership_categories']) : [];
    update_user_meta($user_id, 'membership_categories', $selected_categories);
}
add_action('personal_options_update', 'save_user_membership_category_field');
add_action('edit_user_profile_update', 'save_user_membership_category_field');

// Save fields when adding a new user
function save_user_membership_category_field_new($user_id)
{
...
add_action('user_register', 'save_user_membership_category_field_new');


// Hook to add the Certificate Format page in the admin menu
add_action('admin_menu', 'add_certificate_format_page');

function add_certificate_format_page()
{
    add_menu_page(
        'Certificate Format',        // Page title
        'Certificate Format',        // Menu title
        'manage_options',            // Capability (admins only)
        'certificate-format',        // Menu slug
        'display_certificate_format_page', // Callback function
        'dashicons-awards',          // Icon
        6                            // Position
    );
}

add_action('init', 'create_certificate_format_table');

function create_certificate_format_table()
{
    global $wpdb;
    $table_name = $wpdb->prefix . 'certificate_format'; // Table name with WP prefix

    $charset_collate = $wpdb->get_charset_collate();

    $sql = "CREATE TABLE $table_name (
        id mediumint(9) NOT NULL AUTO_INCREMENT,
        iss_logo VARCHAR(255) DEFAULT '' NOT NULL,
        cohost_logo VARCHAR(255) DEFAULT '' NOT NULL,
        background_option VARCHAR(20) DEFAULT 'color' NOT NULL,
        background_color VARCHAR(20) DEFAULT '' NOT NULL,
        background_image VARCHAR(255) DEFAULT '' NOT NULL,
...
function display_certificate_format_page()
{
    global $wpdb;
    $table_name = $wpdb->prefix . 'certificate_format';

    // Handle form submission
    if (isset($_POST['submit_certificate_format'])) {
        $current_user = wp_get_current_user();
        $updated_by = $current_user->display_name;

        // Initialize an array to hold updated fields
        $update_data = [];

        // Handle ISS Logo Upload
        if (!empty($_FILES['iss_logo']['name'])) {
            $uploaded_file = wp_handle_upload($_FILES['iss_logo'], ['test_form' => false]);
            if (!isset($uploaded_file['error'])) {
                $update_data['iss_logo'] = $uploaded_file['url'];
            }
        }

        // Handle Co-host Logo Upload
        if (!empty($_FILES['cohost_logo']['name'])) {
            $uploaded_file = wp_handle_upload($_FILES['cohost_logo'], ['test_form' => false]);
            if (!isset($uploaded_file['error'])) {
                $update_data['cohost_logo'] = $uploaded_file['url'];
            }
        }

        // Handle Background Option
        if (isset($_POST['background_option'])) {
            $background_option = sanitize_text_field($_POST['background_option']);
            $update_data['background_option'] = $background_option;
            ...
            } elseif ($background_option === 'image' && !empty($_FILES['background_image']['name'])) {
                $uploaded_file = wp_handle_upload($_FILES['background_image'], ['test_form' => false]);
                if (!isset($uploaded_file['error'])) {
                    $update_data['background_image'] = $uploaded_file['url'];
                }
            }
        }

        // Always update the updated_by and created_at fields
        $update_data['updated_by'] = $updated_by;
        $update_data['created_at'] = current_time('mysql');

        // Update the last inserted entry instead of creating a new one
        $last_entry_id = $wpdb->get_var("SELECT id FROM $table_name ORDER BY created_at DESC LIMIT 1");

        if ($last_entry_id && !empty($update_data)) {
            $wpdb->update(
                $table_name,
                $update_data,
                ['id' => $last_entry_id]
        ...
        echo '<div class="updated notice"><p>Certificate format updated successfully!</p></div>';
    }

    // Form Display Section
    wp_enqueue_style('wp-color-picker');
    wp_enqueue_script('wp-color-picker');
    wp_add_inline_script('wp-color-picker', 'jQuery(function($){ $("#custom_color_picker").wpColorPicker(); });');

    if (current_user_can('manage_options')) {
        echo '<div class="wrap">';
        echo '<h1>Certificate Format Page</h1>';
        echo '<h5>For Making Change Please fill whole form with updated data.</h5>';
        echo '<form method="post" enctype="multipart/form-data">';

        // ISS Logo Upload
        echo '<h3>Upload ISS Logo</h3>';
        echo '<input type="file" name="iss_logo" accept="image/*" />';

        // Co-host Logo Upload
        echo '<h3>Upload Co-host Logo</h3>';
        echo '<input type="file" name="cohost_logo" accept="image/*" />';

        // Background Option - Radio
        echo '<h3>Select Background Option</h3>';
        echo '<label><input type="radio" name="background_option" value="color" checked> Select Background Color</label><br>';
        echo '<label><input type="radio" name="background_option" value="image"> Upload Background Image</label>';

        // Color Picker with Predefined Colors
        echo '<div id="color_picker_section" style="margin-top:10px;">';
        echo '<h4>Select Background Color</h4>';
        echo '<input type="text" id="custom_color_picker" name="background_color" value="#ffffff" />';
        echo '<div style="margin-top:10px;">';
        $colors = ['#FF5733', '#33FF57', '#3357FF', '#F1C40F', '#8E44AD', '#34495E', '#E74C3C'];
        foreach ($colors as $color) {
            echo '<div class="color-swatch" data-color="' . $color . '" style="width:40px; height:40px; background-color:' . $color . '; border:2px solid #ccc; cursor:pointer; display:inline-block; margin-right:5px; border-radius:5px;"></div>';
        }
        echo '</div>';
        echo '</div>';

        // Background Image Upload (Hidden by default)
        echo '<div id="image_upload_section" style="margin-top:10px; display:none;">';
        echo '<h4>Upload Background Image</h4>';
        echo '<input type="file" name="background_image" accept="image/*" />';
        echo '</div>';

        // Submit Button
        echo '<br><input type="submit" name="submit_certificate_format" class="button button-primary" value="Save Settings">';
        echo '</form>';

        // Display existing certificate formats below the form
        display_existing_certificate_formats();
        echo '</div>';
    } else {
        wp_die(__('You do not have sufficient permissions to access this page.'));
    }

    // Script for toggling background options
    echo '<script>
        jQuery(document).ready(function () {
            jQuery("#custom_color_picker").wpColorPicker();

            jQuery("input[name=\"background_option\"]").change(function () {
                if (jQuery(this).val() === "color") {
                    jQuery("#color_picker_section").show();
                    jQuery("#image_upload_section").hide();
                } else {
                    jQuery("#color_picker_section").hide();
                    jQuery("#image_upload_section").show();
                }
            });

            jQuery(".color-swatch").click(function () {
                var selectedColor = jQuery(this).data("color");
                jQuery(".color-swatch").css("border", "2px solid #ccc");
                jQuery(this).css("border", "2px solid #000");
                jQuery("#custom_color_picker").wpColorPicker("color", selectedColor);
            });
        });
    </script>';
}
...
// Function to display existing certificate formats
function display_existing_certificate_formats()
{
    global $wpdb;
    $table_name = $wpdb->prefix . 'certificate_format';
    ...
    if (!empty($results)) {
        echo '<h2>Existing Certificate Formats</h2>';
        echo '<table class="wp-list-table widefat fixed striped">';
        echo '<thead><tr><th>ISS Logo</th><th>Co-host Logo</th><th>Background</th><th>Updated By</th><th>Created At</th></tr></thead><tbody>';

        foreach ($results as $row) {
            echo '<tr>';
            echo '<td>' . (!empty($row->iss_logo) ? '<img src="' . esc_url($row->iss_logo) . '" width="50" />' : 'N/A') . '</td>';
            echo '<td>' . (!empty($row->cohost_logo) ? '<img src="' . esc_url($row->cohost_logo) . '" width="50" />' : 'N/A') . '</td>';
            echo '<td>';
            if ($row->background_option == 'color') {
                echo '<div style="width:50px; height:50px; background-color:' . esc_attr($row->background_color) . ';"></div>';
            } elseif ($row->background_option == 'image') {
                echo (!empty($row->background_image) ? '<img src="' . esc_url($row->background_image) . '" width="50" />' : 'N/A');
            }
            echo '</td>';
            echo '<td>' . esc_html($row->updated_by) . '</td>';
            echo '<td>' . esc_html($row->created_at) . '</td>';
            echo '</tr>';
        }
        ...
        echo '<p>No certificate formats found.</p>';
    }
}
...
function custom_attendance_admin_page()
{
    ...
    add_menu_page(
        ...
        'attendance_page',          // Menu Slug
        'display_abstract_submissions',  // Callback Function
        'dashicons-calendar-alt',   // Icon
        20                          // Position
    );
}

add_action('admin_menu', 'custom_attendance_admin_page');
function register_co_author_attendance_page()
{
    add_menu_page(
        'Co-Author Attendance',    // Page title
        'Co-Author Attendance',    // Menu title
        'manage_options',          // Capability
        'co-author-attendance',    // Menu slug (should match the button URL)
        'co_author_attendance_page', // Callback function to display the page
        'dashicons-groups',        // Icon
        20                         // Position
    );
}
add_action('admin_menu', 'register_co_author_attendance_page');

function display_abstract_submissions()
{
    global $wpdb;
    $table_name = $wpdb->prefix . 'abstract_submissions'; // Table name with prefix
    ...
    // Handle form submission
    if (isset($_POST['save_attendance']) && !empty($_POST['present'])) {
        $new_present_ids = array_map('intval', $_POST['present']);

        foreach ($new_present_ids as $checked_id) {
            $wpdb->update(
                $table_name,
                [
                    'present' => 'yes',
                    'present_marked_at' => current_time('mysql'),
                    'present_marked_by' => $current_user_name
                ],
                ['id' => $checked_id],
                ['%s', '%s', '%s'],
                ['%d']
        ...
        echo '<script>window.location.reload();</script>'; // Refresh page after submission
    }

    // Query to fetch required data using JOIN and filter results if search is applied
    $query = "SELECT a.id, a.author_name, u.user_email, a.abstract_title, a.present, a.present_marked_at, a.present_marked_by 
              FROM $table_name a
              LEFT JOIN $users_table u ON a.author_id = u.ID";

    if (!empty($search_query)) {
        $query .= $wpdb->prepare(" WHERE a.author_name LIKE %s OR u.user_email LIKE %s OR a.abstract_title LIKE %s", "%$search_query%", "%$search_query%", "%$search_query%");
    }

    $results = $wpdb->get_results($query);

    echo '<div class="wrap"><h1>Abstract Submissions</h1>';

    // Search Form
    echo '<form method="GET" action="" class="custom-admin-search-form search-form-wrapper">';
    echo '<input type="hidden" name="page" value="' . esc_attr($_GET['page']) . '">';
    echo '<div class="search-input-wrapper">';
    echo '<input type="text" name="search" class="search-input-field" value="' . esc_attr($search_query) . '" placeholder="Search by author name, email, or abstract title">';
    echo '</div>';
    echo '<div class="search-button-wrapper">';
    echo '<input type="submit" class="button button-primary search-submit-button" value="Search">';
    echo '<a href="' . admin_url('admin.php?page=attendance_page') . '" class="button search-reset-button">Reset</a>';
    echo '</div>';
    echo '</form><br>';
    ...
    echo '<thead><tr>
            ...
            <th>Marked At</th>
            <th>Marked By</th>
            <th>Co-Author Attendance</th>
          </tr></thead><tbody>';

    if (!empty($results)) {
        foreach ($results as $row) {
            $checked = ($row->present == 'yes') ? 'checked' : ''; // Check if already marked as present
            $disabled = ($row->present == 'yes') ? 'disabled' : '';
            $marked_at = !empty($row->present_marked_at) ? $row->present_marked_at : '-';
            $marked_by = !empty($row->present_marked_by) ? $row->present_marked_by : '-';
            $co_author_url = admin_url("admin.php?page=co-author-attendance&id={$row->id}");
            $co_author_action = ($row->present == 'yes')
                ? "<a href='{$co_author_url}' class='button button-secondary'>Mark Co-Author Attendance</a>"
                : "<span style='color: red;'>Please mark author attendance to activate this.</span>";
            ...
            echo "<tr>
                    ...
                    <td><input type='checkbox' name='present[]' value='{$row->id}' $checked $disabled></td>
                    <td>{$marked_at}</td>
                    <td>{$marked_by}</td>
                    <td>{$co_author_action}</td>
                  </tr>";
        }
    } else {
    ...
    echo '</tbody></table>';
    echo '<br><input type="submit" name="save_attendance" class="button button-primary" value="Save Attendance">';
    echo '</form></div>';
}

function co_author_attendance_page()
{
    ...
    $abstract_id = isset($_GET['id']) ? intval($_GET['id']) : 0;

    if (!$abstract_id) {
        echo '<div class="notice notice-error"><p>Invalid abstract ID.</p></div>';
        return;
    }

    // Get only 'Valid' co-authors from the co-author certificates table
    $co_author_certificates_table = $wpdb->prefix . 'co_author_certificates';
    $co_authors = $wpdb->get_results($wpdb->prepare(
        "SELECT id, user_id, membership_id, submitter_id, status,present, certificate_link, certificate_id, created_at 
         FROM $co_author_certificates_table 
         WHERE abstract_id = %d AND status = 'Valid'",
        $abstract_id
    ));

    echo '<div class="wrap"><h1>Co-Author Attendance & Certificates</h1>';

    if (empty($co_authors)) {
        echo '<p>No valid co-authors found for this abstract.</p>';
        return;
    }

    // Handle attendance marking
    if (isset($_POST['save_co_author_attendance']) && !empty($_POST['co_present'])) {
        foreach ($_POST['co_present'] as $co_id) {
            $wpdb->update(
                $co_author_certificates_table,
                ['present' => 'present'], // Update status to 'present'
                ['id' => intval($co_id)],
                ['%s'],
                ['%d']
            );
        ...
        echo '<script>window.location.reload();</script>'; // Refresh after saving
    }
    ...
    foreach ($co_authors as $co_author) {
        $checked = ($co_author->present == 'present') ? 'checked' : '';
        $disabled = ($co_author->present == 'present') ? 'disabled' : '';

        // Show certificate link if available
        $certificate_link = !empty($co_author->certificate_link)
            ? "<a href='{$co_author->certificate_link}' target='_blank'>View Certificate</a>"
            : "No Certificate";

        echo "<tr>
                <td>{$co_author->id}</td>
                <td>{$co_author->user_id}</td>
                <td>{$co_author->membership_id}</td>
                <td>{$co_author->submitter_id}</td>
                <td>{$co_author->status}</td>
                <td>{$certificate_link}</td>
                <td>{$co_author->created_at}</td>
                <td><input type='checkbox' name='co_present[]' value='{$co_author->id}' $checked $disabled></td>
              </tr>";
    }

    echo '</tbody></table>';
    echo '<br><input type="submit" name="save_co_author_attendance" class="button button-primary" value="Save Attendance">';
    echo '</form></div>';
}
